...
export function ProjectsSection() {
  ...
  return <section className="projects section-shell" id="projects"><div className="projects-heading"><div><p className="eyebrow">02 / Selected work</p><h2>Built with<br /><em>intention.</em></h2></div><p>Selected explorations across product, platform and experience.</p></div><div className="project-list">{projects.map((project, index) => <div key={project.number}><motion.article className={`project-row ${index % 2 ? "reverse" : ""}`} initial={{ opacity: 0, y: reduced ? 0 : 34, scale: reduced ? 1 : .985, clipPath: reduced ? "none" : "inset(0 0 9% 0)" }} whileInView={{ opacity: 1, y: 0, scale: 1, clipPath: "inset(0 0 0% 0)" }} viewport={{ once: true, amount: .16 }} transition={{ duration: reduced ? 0 : .75, ease: [0.22, 1, 0.36, 1] }}>{project.number === "01" ? <EvolystStayGallery /> : project.number === "02" ? <BusinessAdminGallery /> : project.number === "03" ? <RestaurantPosGallery /> : <div className={`project-art ${project.accent}`} aria-label={`${project.title} project preview`}><span className="project-orbit" /><span className="project-art-title">{project.category}</span><div className="mock-window"><i /><i /><i /><b /></div></div>}<div className="project-copy"><p className="project-number">{project.number} <span>{project.year}</span></p><h3>{project.title}</h3><p>{project.description}</p><ul className="stack">{project.stack.map((item) => <li key={item}>{item}</li>)}</ul><div className="project-actions"><button className="case-study-trigger" type="button" onClick={() => setActiveCaseStudy(project)}>View case study <ArrowUpRight size={16} /></button><a href={project.githubUrl || "https://github.com/ZhiHin"} aria-label={`Source code for ${project.title}`} target="_blank" rel="noreferrer"><Github size={18} /></a></div></div></motion.article>{project.number === "01" && <EvolystConsoleGallery />}</div>)}</div>{activeCaseStudy && <ProjectCaseStudyDialog project={activeCaseStudy} onClose={() => setActiveCaseStudy(null)} />}</section>;
}